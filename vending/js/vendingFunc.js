class VendingMachineFunc {
    ...
    bindEvents() {
         ...
         * 획득한 음료의 금액을 모두합하여 총금액을 업데이트합니다.
         */
        ...
        this.btnGet.addEventListener('click',()=>{
            ...
            //획득한 음료리스트를 순환하면서 총금액을 계산한다.
            this.getList.querySelectorAll('li').forEach((itemGet)=>{
                total += parseInt(itemGet.dataset.price) * parseInt(itemGet.querySelector('strong').firstChild.textContent)
            })

            this.txtTotal.textContent = `총금액: ${new Intl.NumberFormat().format(total)} 원`
        })
    }
}
